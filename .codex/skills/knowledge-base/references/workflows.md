...
- 出題レベルを使い分ける
  - レベル1: 1問1答形式。用語や要点を1〜2文で短く答える
  - レベル2: レベル1とレベル3の中間。概念と重要になる場面を説明する
  - レベル3: 総合説明形式。概念、重要になる場面、実務やコードでの扱いまで説明する
- `惜しい` `不正解` `スキップ` は `#要復習` を必ず付ける
- `#要復習` を保存するときは、対象ノートと変更内容を示して確認を取る
- `#要復習` は日をまたいだ2回連続正解で外す
- 同じ日に何度正解しても `review_streak` は最大1までとし、タグは外さない
