...
`0` や空文字 `""` とは意味が違います。
SQLでは、`NULL` は通常の値のように `=` で比較せず、`IS NULL` を使って判定します。PostgreSQL公式ドキュメントでも、`NULL` 判定には `IS NULL` / `IS NOT NULL` を使う構文が説明されています。([PostgreSQL][2])
...
TypeScriptでは、`strictNullChecks` が `true` のとき、`null` と `undefined` はそれぞれ別の型として扱われます。([typescriptlang.org][3])
...
JavaScriptでは、`typeof null` は `"object"` になるため、`null` の確認には `=== null` を使う必要があります。([MDNウェブドキュメント][4])
...
`0` や空文字 `""` は有効な値として扱われるため、`||` と挙動が異なります。([MDNウェブドキュメント][5])
...
[2]: https://www.postgresql.org/docs/current/functions-comparison.html?utm_source=chatgpt.com "18: 9.2. Comparison Functions and Operators"
...
[5]: https://developer.mozilla.org/ja/docs/Web/JavaScript/Reference/Operators/Nullish_coalescing?utm_source=chatgpt.com "ヌル値合体演算子 (??) - JavaScript - MDN Web Docs"
