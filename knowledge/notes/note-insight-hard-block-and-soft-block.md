...
## Body
ハードブロックとソフトブロックの違いは、認可違反のときにどこまで処理を止めるかにあります。ハードブロックは「見せてはいけない」「進ませてはいけない」ときに処理全体を止めます。ソフトブロックはページは表示しつつ、ボタンを出さない、案内を出すなど、見せ方で制御する考え方です。認可の厳しさと UX のバランスを考えるときの軸になります。
