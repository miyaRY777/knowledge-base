---
id: note-insight-motherboard
title: "マザーボードは各部品をつなぐ基盤"
created: 2026-04-15
source: [[2026-04-15_insight_memory-storage-motherboard-basics]]
review_streak: 0
quiz_fail_log: []
---

## Summary
- マザーボードは CPU、メモリ、ストレージなどを取り付ける土台です。
- 各部品が通信できるようにつなぐ役割を持ちます。
- コンピュータ全体の接続基盤として働きます。

## Tags
#computer #hardware #motherboard

## Links
- [[note-insight-cpu]]
- [[note-insight-main-memory]]
- [[note-insight-secondary-storage]]
- [[note-insight-bus]]
- [[note-insight-slot-and-socket]]

## Body
マザーボードは、コンピュータの主要な部品を取り付け、相互に接続するための基盤です。CPU やメモリ、ストレージはそれぞれ単独では十分に動けず、マザーボードを通してデータのやり取りや電力供給を受けながら連携します。コンピュータ全体の土台として理解すると整理しやすいです。

## 言語化

結論：
理由：
具体例：
結論（まとめ）：
